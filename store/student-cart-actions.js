  ...
  return async (dispatch) => {
    const postHandler = async (addCartParams) => {
      ...
      const res = await fetch(
        `https://backend-mealtrain-order-service-production.up.railway.app/mealtrain/students/${addCartParams.studentId}/cart/`,
        {
          method: "POST",
...
export const deleteCart = (deleteCartParams) => {
  return async (dispatch) => {
    const deleteHandler = async (deleteCartParams) => {
      const res = await fetch(
        `https://backend-mealtrain-order-service-production.up.railway.app/mealtrain/students/${deleteCartParams.studentId}/cart/${deleteCartParams.cartId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
...
export const updateCart = (updatedCartParams) => {
  return async (dispatch) => {
    const putHandler = async (updatedCartParams) => {
      const res = await fetch(
        `https://backend-mealtrain-order-service-production.up.railway.app/mealtrain/students/${updatedCartParams.studentId}/cart/${updatedCartParams.cartId}`,
        {
          method: "PUT",
